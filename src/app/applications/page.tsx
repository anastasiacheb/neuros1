'use client';
import { Benefits, FAQ, ApplicationCard } from '@/components';
import { ApplicationsData } from '@/data';
import { useState } from 'react';

export default function Page() {
  const ApplicationsCategories = [...new Set(ApplicationsData.flatMap((item) => item.industries))].sort();
  const [isActive, setIsActive] = useState(0);
  const ApplicationsDataFiltered =
    isActive === 0
      ? ApplicationsData
      : ApplicationsData.filter((item) => item.industries.includes(ApplicationsCategories[isActive]));
